...
def remove_alert(update, context):
    ...
    try:
        ...
        if 1 <= removal <= len(notices[chat_id]):
            del notices[chat_id][removal - 1]
            new_response = "Successfully removed."
            update.message.reply_text(new_response)
        else:
            raise IndexError
    except:
        new_response = """Failed. Please key in a valid index.\n\n"""
        new_response += 'Your current alerts are: \n\n'
        i = 1
        for alert in notices[chat_id]:
            new_response += f"{i}. {alert[0]} {alert[1]} {alert[2]} \n"
            i += 1
        update.message.reply_text(new_response)
    ...
    if len(context.args) == 3:
        ...
        if istickervalid(ticker) and issignvalid(sign) and ispricevalid(price):
            ...
            try:
                ...
                update.message.reply_text(response)
        # changed
            context.job_queue.run_repeating(stocknotif, interval=15, first=5, context=[ticker, sign, price,
